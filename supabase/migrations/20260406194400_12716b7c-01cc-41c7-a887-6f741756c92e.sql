...
CREATE TABLE public.saved_ideas (
  ...
  quote_suggestions TEXT[] DEFAULT '{}',
  ai_product_name TEXT,
  ai_description TEXT,
  competitor_gaps TEXT[] DEFAULT '{}',
  notes TEXT,
  priority_score INTEGER DEFAULT 0,
  built_status BOOLEAN DEFAULT false,
  revenue_price NUMERIC(10,2),
  revenue_sales INTEGER DEFAULT 0,
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);
...
CREATE POLICY "Users can update their own saved ideas" ON public.saved_ideas FOR UPDATE USING (auth.uid() = user_id);
